#!/usr/bin/env python3
"""Generate Module #3 reports for Module #2 configuration analysis.

The script uses the same neutral prompt for all evidence conditions:

- rgb_only
- reba_only
- full_module2

For rgb_only, the referenced local image is attached as a vision input. Use
--dry-run to verify prompt construction and output paths without API calls.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="append",
        choices=CONDITIONS,
        help="Condition to run. Repeat for multiple. Default: all conditions.",
    )
    parser.add_argument(
        "--case-id",
        dest="case_ids",
        action="append",
        help="Optional case filter such as case_01. Repeat for multiple.",
    )
    ...
    parser.add_argument("--structured-max-tokens", type=int, default=1600)
    parser.add_argument("--natural-max-tokens", type=int, default=1800)
    ...
    parser.add_argument(
        "--retry-errors",
        action="store_true",
        help="Skip successful outputs, but overwrite dry-run/error/incomplete outputs.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Write prompts and placeholder outputs without calling the API.",
    )
    parser.add_argument(
        "--output-root",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=m2_root / "prompts" / "structured_neutral_contribution.txt",
    )
    parser.add_argument(
        "--natural-prompt-file",
        type=Path,
        default=m2_root / "prompts" / "natural_neutral_contribution.txt",
    )
...
def run_report_type(args: argparse.Namespace, report_type: str, condition: str) -> None:
    ...
    manifest = {
        "analysis": "module2_configuration_contribution",
        "report_type": report_type,
        "condition": condition,
        "model": args.model,
        "runs": args.runs,
...
def main() -> None:
    ...
    print("Configuration report generation complete.")
