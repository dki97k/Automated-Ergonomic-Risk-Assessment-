#!/usr/bin/env python3
"""Draft-label Module #2 configuration natural-report claims.

Labels are conservative evidence-audit labels:

- supported: directly present in the condition evidence, or a cautious
  biomechanical interpretation tied to visible/numerical evidence
- unsupported: introduces information not available under the condition
- contradiction: conflicts with a numerical value or evidence-availability fact

These are initial evidence-audit labels for configuration analysis, not final
reviewed labels.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--annotation-csv",
        type=Path,
        default=m2_root / "results" / "evaluation" / "claim_annotation_sheet.csv",
    )
    parser.add_argument(
        "--output-csv",
        type=Path,
...
def claim_evidence_needs(text: str) -> set[str]:
    ...
    if has_any(text, ("rgb", "image", "visible", "observed", "appears", "work environment")):
        needs.add("rgb")
    return needs
...
def missing_statement_matches(text: str, available: dict[str, bool]) -> bool:
    ...
    if has_any(text, ("rgb", "image", "visual", "work environment", "task details")) and not available["rgb"]:
        return True
    if has_any(text, ("numerical posture", "time-series")):
        return True
    return False
...
def visible_support(case_id: str, text: str) -> bool:
    ...
    if has_any(text, ("posture", "awkward", "neutral", "strain", "ergonomic risk")):
...
def full_or_reba_factor_supported(payload: dict[str, Any], text: str) -> bool:
    ...
    if has_any(text, ("reba", "risk level", "posture score", "medium", "moderate")):
    ...
    if has_any(text, ("duration", "static", "sustained", "prolonged")):
...
def unsupported_specificity(text: str, condition: str) -> str | None:
    ...
    if "training" in text:
        return "training recommendation is not supported by condition evidence"
    if has_any(text, ("rest break", "breaks", "rotation", "work schedule")):
        return "break/rotation/work-schedule details are not provided"
    if condition != "full_module2" and has_any(
        text,
        ("tool", "tools", "equipment", "platform", "support"),
    ):
        return "specific tool/equipment recommendation exceeds limited condition evidence"
    if condition == "rgb_only" and has_any(text, ("proper lifting", "lifting technique")):
        return "lifting technique is not established by the RGB evidence"
    return None


def missing_positive_evidence_reason(
    text: str,
    ...
    if not missing:
        return None
    if condition == "rgb_only" and needs == {"rgb"}:
        return None
    if condition == "rgb_only" and missing:
        return "claim uses evidence absent from RGB-only condition: " + ", ".join(missing)
    if condition == "reba_only":
        meaningful_missing = [need for need in missing if need != "rgb"]
        if meaningful_missing:
            return "claim uses evidence absent from REBA-only condition: " + ", ".join(meaningful_missing)
    return None


def evidence_note(payload: dict[str, Any], case_id: str, condition: str) -> str:
    if condition == "rgb_only":
        return RGB_OBSERVATIONS.get(case_id, {}).get("note", "RGB image evidence reviewed.")
    available = evidence_available(payload)
    if condition == "reba_only":
        final = payload.get("posture_summary", {}).get("final_reba", {})
        return (
            "REBA-only evidence: final REBA mean/p90/max="
    ...
    repetition = payload.get("repetition_summary", {})
    return (
        "Full Module #2 evidence: final REBA mean/p90/max="
        f"{final.get('mean')}/{final.get('p90')}/{final.get('max')}; "
...
def label_claim(row: dict[str, str], payload: dict[str, Any]) -> tuple[str, str, str]:
    ...
    if bad_numbers:
        return (
            "contradiction",
            evidence_note(payload, case_id, condition),
            "Initial audit: numeric value(s) not supported by condition evidence: "
            + ", ".join(bad_numbers),
        )
    ...
    specificity = unsupported_specificity(text, condition)
    if specificity:
        return (
            "unsupported",
            evidence_note(payload, case_id, condition),
    ...
    missing_reason = missing_positive_evidence_reason(text, available, condition)
    if missing_reason:
        return (
            "unsupported",
            evidence_note(payload, case_id, condition),
    ...
    if condition == "rgb_only":
        if visible_support(case_id, text):
            return (
                "supported",
                evidence_note(payload, case_id, condition),
                "Initial audit: visible posture/context supports this cautious claim.",
            )
        if has_any(text, ("additional evidence", "evidence required")):
            return (
                "supported",
                evidence_note(payload, case_id, condition),
                "Initial audit: additional evidence need is consistent with RGB-only input.",
            )
        return (
            "unsupported",
            evidence_note(payload, case_id, condition),
            "Initial audit: claim is not directly supported by the single RGB frame.",
        )

    if full_or_reba_factor_supported(payload, text):
        return (
            "supported",
            evidence_note(payload, case_id, condition),
            "Initial audit: claim is grounded in available numerical evidence.",
        )

    if has_any(text, ("neutral posture", "reduce strain", "ergonomic intervention", "improve")):
        return (
            "supported",
            evidence_note(payload, case_id, condition),
            "Initial audit: recommendation is a general ergonomic target tied to identified posture risk.",
        )
